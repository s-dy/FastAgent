...
from fastagent.core.parser import json_output_parser
...
class Planner:
    ...
    def plan(self, question: str, **kwargs) -> List[str]:
        ...
        monitor_task_status("--- 正在生成计划 ---")
        response = self.llm_client.invoke(messages, **kwargs) or ""
        if isinstance(response,AIMessage):
            response_text = response.content
        else:
            response_text = "Error：在计划生成时返回了工具调用消息"
            monitor_task_status("Error：在计划生成时返回了工具调用消息",level='ERROR')
        monitor_task_status(f"✅ 计划已生成:\n{response_text}")

        try:
            plan = json_output_parser(response_text)
            return plan if isinstance(plan, list) else []
        except Exception as e:
            monitor_task_status(f"❌ 解析计划时发生未知错误: {e}",level='ERROR')
            monitor_task_status(f"原始响应: {response_text}",level='WARNING')
            return []
    ...
    def execute(self, question: str, plan: List[str], **kwargs) -> str:
        ...
        for i, step in enumerate(plan, 1):
            ...
            if isinstance(response,AIMessage):
                ...
                response_text = "Error：在计划执行时返回了工具调用消息"
                monitor_task_status("Error：在计划执行时返回了工具调用消息",level='ERROR')

            history += f"步骤 {i}: {step}\n结果: {response_text}\n\n"
            final_answer = response_text
            monitor_task_status(f"✅ 步骤 {i} 已完成，结果: {final_answer}")
