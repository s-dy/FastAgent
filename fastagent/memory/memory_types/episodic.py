...
class EpisodicMemory(BaseMemory):
    ...
    def __init__(self, config: MemoryConfig, storage_backend, doc_store, vector_store):
    ...
    def add(self, memory_item: MemoryItem) -> str:
        ...
        user_id = memory_item.metadata.get("user_id", "default_user_id")
        context = memory_item.metadata.get("context", {}) # 上下文信息
        outcome = memory_item.metadata.get("outcome", "") # 结果
        participants = memory_item.metadata.get("participants", []) # 参与者
        tags = memory_item.metadata.get("tags", []) # 标签
    ...
    def forget(self, strategy: str = "importance_based", threshold: float = 0.1, max_age_days: int = 30) -> int:
        ...
        for episode_id in to_remove:
            if self.remove(episode_id):
                forgotten_count += 1
                monitor_task_status(f"情景记忆硬删除: {episode_id[:8]}... (策略: {strategy})")
    ...
    def get_all(self) -> List[MemoryItem]:
        ...
        for episode in self.episodes:
            memory_item = MemoryItem(
                ...
                metadata={"user_id": episode.user_id,"session_id": episode.session_id,"context": episode.context,"outcome": episode.outcome},
            )
    ...
    def find_patterns(self, user_id: str = None, min_frequency: int = 2) -> List[Dict[str, Any]]:
        ...
        for episode in episodes:
            # 提取关键词
            words = split_sentence(episode.content)
            print(words)
            for word in words:
